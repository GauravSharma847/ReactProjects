...
const Videos = () => {
  ...
  const [videoSrc, setVideoSrc] = useState(videosArr[0]);  // by default it plays the first video

  return (
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>   {/* this vstack has media player and heading_description*/}
        <video
          controls   // it gives the controls
          controlsList="nodownload"   // it disables the download on player
          src={videoSrc}
          style={{
            width: '100%',
          }}
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>  {/* this vstack has heading and video description*/}
      ...
      <VStack                  
        ...
        spacing={'8'}
        overflowY={'auto'}
      >     {/* this v stack contains all videos*/}
        {videosArr.map((item, index) => (    // as the button is clicked according to mapping videos is played from the videoArr
          <Button
            variant={'ghost'}
            colorScheme={'purple'}
